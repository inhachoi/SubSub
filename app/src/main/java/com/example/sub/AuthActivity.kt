...
import com.example.sub.databinding.ActivityAuthBinding
import com.example.sub.LoginApplication
import com.example.sub.MysubActivity
...
class AuthActivity : AppCompatActivity() {
                ...
                LoginApplication.auth.signInWithCredential(credential)
                    . addOnCompleteListener(this){
                        if(task.isSuccessful){
                            LoginApplication.email = account.email
                            changeVisibility("login")
                        }else {
                            changeVisibility("logout")
                        }
                ...
                .addOnCompleteListener(this){ task ->
                    ...
                    if(task.isSuccessful){
                        if(LoginApplication.checkAuth()){
                            ...
                            Toast.makeText(baseContext, "전송된 메일로 이메일 인증이 되지 않습니다", Toast.LENGTH_SHORT).show()
                        }
                    }else{
    ...
    fun changeVisibility(mode: String){
        if(mode === "login"){
            binding.run {
                authMainTextView.text = "${LoginApplication.email} 님 반갑습니다."
